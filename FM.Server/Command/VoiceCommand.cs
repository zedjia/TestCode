...
namespace FM.Server.Command
{
    ...
    public sealed class VoiceCommand : ICommand<AsyncBinaryCommandInfo>, ISocketServiceContainer
    {
        /// <summary>
        /// 返回服务名称
        /// </summary>
        ...
        public void ExecuteCommand(IConnection connection, AsyncBinaryCommandInfo commandInfo)
        {
            ...
            if (form.OnlineUsers.Any(i => i.LoginID == dto.InChargeUser))//InChargeUser:消控室userId
            {
                //var loginId = form.OnlineUsers.FirstOrDefault(i => i.IsLogin).LoginID;
                form.CurrentTaskQueue.TryAdd(dto.InChargeUser.ToString(), string.Format("{0},{1}", CommonCommands.Voice.ToString(), dto.RoomId)); 

                commandInfo.Reply(connection, System.Text.Encoding.Default.GetBytes("已经收到语音指令"));
                form.DisplayMsg(string.Format("服务器已经收到{0}语音指令!,房间id:{1}", connection.ConnectionID, dto.RoomId));
                ...
                if (Cjqdto!=null)
                {
                    ...
                    form.DisplayMsg(string.Format("服务器已经收到{0}语音指令!,转发给设备采集器:{1}", connection.ConnectionID, Cjqdto.UserName));
                }
                else
                {
                    form.DisplayMsg(string.Format("服务器已经收到{0}语音指令!,但是未找到该消控室的采集器登录", connection.ConnectionID));
                }
            }
            else
            {
                form.DisplayMsg(string.Format("服务器已经收到{0}语音指令!,但是用户{1}不存在", connection.ConnectionID, dto.InChargeUser));
                commandInfo.Reply(connection, System.Text.Encoding.Default.GetBytes("已经收到语音指令,用户不存在"));
